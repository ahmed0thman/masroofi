...
export default function App() {
  ...
  const ensureMicrophonePermission = async (): Promise<boolean> => {
    if (Platform.OS === 'web') {
      Alert.alert('Unsupported Platform', 'Real-time transcription is not available on the web.');
      return false;
    }
  ...
  const handleDeleteModel = (modelId: string) => {
    if (isRealtimeActive || isTranscribing) {
      Alert.alert('Busy', 'Please stop any active transcription before deleting models.');
      return;
    }
  ...
  const transcribeAudio = async () => {
    ...
    try {
      ...
      const expoAudioPath = new File(Paths.document, APP_DIRECTORY_NAME, 'jfk.wav');
  ...
  const transcriptionStatusText = isTranscribing ? 'Processing sample…' : 'Idle';
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statusRow}>
            <View style={[styles.statusCard, whisperContext && styles.statusCardActive]}>
              <Text style={styles.statusTitle}>Model</Text>
              <Text style={styles.statusValue}>{whisperStatusText}</Text>
            </View>
            <View style={[styles.statusCard, isRealtimeActive && styles.statusCardActive]}>
        ...
        <View style={[styles.card, isRealtimeActive && styles.liveCard]}>
          <View style={styles.cardHeader}>
            ...
            {isRealtimeActive ? <Text style={styles.liveBadge}>Live</Text> : null}
          </View>
          <Text style={[styles.cardText, !realtimeResult && styles.placeholderText]}>
            {realtimeResult ||
        ...
        <View style={styles.section}>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity
              ...
              onPress={isRealtimeActive ? stopRealtimeTranscription : startRealtimeTranscription}
              disabled={!whisperContext}
            >
              <Text style={isRealtimeActive ? styles.stopButtonText : styles.secondaryButtonText}>
              ...
              return (
                <View key={modelId} style={styles.storageRow}>
                  <View style={styles.storageMeta}>
                    ...
                    <Text style={styles.storageDetails}>Size {formatBytes(info.size)}</Text>
                    <Text style={styles.storagePath} numberOfLines={1} ellipsizeMode="middle">
                      {info.path}
                    </Text>
                  </View>
                  <TouchableOpacity onPress={() => handleDeleteModel(modelId)} disabled={deleting}>
                    <Text style={[styles.deleteLink, deleting && styles.deleteDisabled]}>
