...
export const WHISPER_MODELS: WhisperModel[] = [
  ...
  {
    id: 'large-turbo',
    label: 'Large Turbo (Best Accuracy)',
      ...
      try {
        const downloadResumable = createDownloadResumable(
          ...
          (progressData: DownloadProgressData) => {
            ...
            console.log(`Download progress for ${model.id}: ${(fraction * 100).toFixed(1)}%`);
          },
        );
      ...
      if (currentModelId === modelId) {
        if (whisperContext?.release) {
          try {
            ...
            console.warn('Failed to release Whisper context during model deletion:', releaseError);
          }
        }
        setWhisperContext(null);
        setCurrentModelId(null);
        ...
        const entries = await Promise.all(
          WHISPER_MODELS.map(async (model) => {
            ...
            try {
              ...
              console.warn(`Failed to stat existing model file ${model.id}:`, statError);
              return {
                id: model.id,
                info: {
                  path: file.uri,
